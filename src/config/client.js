...
// Inicializar cliente bajo demanda (no automático)
let isInitialized = false;

const initializeClient = async () => {
  if (isInitialized) {
    console.log("[WHATSAPP] Cliente ya inicializado");
    return;
  }
  try {
    ...
    console.error("[WHATSAPP] Error inicializando cliente:", err.message);
    throw err;
  }
};

// Auto-inicializar solo si hay sesión guardada
const checkAndInitialize = async () => {
  try {
    // Verificar si existe localStorage con sesión
    if (localAuth.clientId) {
      console.log("[WHATSAPP] Sesión existente detectada, inicializando...");
      await initializeClient();
    }
  } catch (err) {
    console.warn("[WHATSAPP] No se pudo auto-inicializar:", err.message);
  }
};

export { client, getIsReady, getReadyAt, getLastQR, initializeClient, checkAndInitialize };
